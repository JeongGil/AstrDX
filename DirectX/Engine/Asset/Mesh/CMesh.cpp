...
void CMesh::RenderInstancing(const FVertexBuffer& InstancingBuffer, int Count)
{
	if (Count == 0)
	{
		return;
	...
	UINT Stride[2] = { VertexBuffer.Size, static_cast<UINT>(InstancingBuffer.Size) };
	UINT Offset[2] = {};
	...
	if (MeshSlots.empty())
	{
		...
		Context->DrawInstanced(VertexBuffer.Count, Count, 0, 0);
	}
	else
	{
		for (const auto& MeshSlot : MeshSlots)
		{
			...
			Context->DrawIndexedInstanced(MeshSlot->IndexBuffer.Count, Count, 0, 0, 0);
		}
	}
}

void CMesh::RenderInstancing(size_t SlotIndex, const FVertexBuffer& InstancingBuffer, int Count)
{
	if (Count == 0)
	{
		return;
