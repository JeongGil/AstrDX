...
CShaderTileMapLineInstancing::CShaderTileMapLineInstancing()
{
}
...
bool CShaderTileMapLineInstancing::Init(const std::string& PathName)
{
	if (!LoadVertexShader("TileMapLineInstancingVS", TEXT("TileMap.hlsl"), PathName))
	{
		return false;
	}

	if (!LoadPixelShader("TileMapLineInstancingPS", TEXT("TileMap.hlsl"), PathName))
	{
		return false;
	}
